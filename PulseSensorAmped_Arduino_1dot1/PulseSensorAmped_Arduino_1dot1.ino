...
volatile int BPM;                   // used to hold the pulse rate of pulse 1
volatile int BPM2;                  // pulse rate 2
volatile int Signal;                // holds the incoming raw data from pulse 1
volatile int Signal2;               // from pulse 2
...
void loop(){
  ...
  if (QS == true){                       // Quantified Self flag is true when arduino finds a heartbeat
    ...
    sendDataToProcessing('T', Signal2);     // send Processing the raw Pulse Sensor data
  if (QS2 == true){                       // Quantified Self flag is true when arduino finds a heartbeat
        fadeRate = 255;                  // Set 'fadeRate' Variable to 255 to fade LED with pulse
        sendDataToProcessing('R',BPM2);   // send heart rate with a 'B' prefix
        //sendDataToProcessing('Q',IBI);   // send time between beats with a 'Q' prefix
